...
class G1MPPIController:
    ...
    Implements a Model Predictive Path Integral control law to navigate 
    ...
    def __init__(self, device="cuda", local_target=None, obstacles=None, global_path=None, dt=C.MPPI_DT):
        """
        Initialize the MPPI solver with dynamics and cost objectives.
        
        Args:
            obstacles: obstacle coordinates.
            global_path: Reference waypoints from A* planner.
        ...
        # STORE ORIGINAL GLOBAL PATH
        if global_path is not None:
            self.global_path = torch.tensor(global_path, dtype=torch.float32, device=device)
            # Initialize the initial short path segment (prevent errors if command hasn't run yet)
        ...
        self.mppi = MPPI(
            self.dynamics,      # Transition model for the G1 robot
            self.cost,          # Objective function to minimize
            nx=3,               # State dim: [x, y, yaw]
    ...
    def dynamics(self, state, action):
        """
        Discrete-time Kinematic Model for G1.
        Performs Coordinate Transformation from Body Frame to World Frame.
        
        State: [x, y, yaw]
        Action: [vx, vy, omega] (Velocities in robot local frame)
        """
        x, y, yaw = state[:, 0], state[:, 1], state[:, 2] 
        vx, vy, omega = action[:, 0], action[:, 1], action[:, 2] 
        
        # Apply Euler integration for state updates
    ...
    def cost(self, state, action):
        ...
        # Calculates Euclidean distance to the local waypoint
        dx = self.local_target[0] - state[:, 0]
        dy = self.local_target[1] - state[:, 1]
        dist_to_target = torch.sqrt(dx**2 + dy**2)
        
        # Heading error: Align robot yaw with the vector pointing to target
        ...
        if self.obstacles is not None:
            # Batch computation of distances to all obstacles
        ...
        # Extreme cost if the robot enters the collision zone
        ...
        # Soft penalty: Progressive repulsion as the robot nears an obstacle
        obstacle_cost += torch.where(
        ...
        # Speed reward: Incentivize moving forward to reach the goal faster
        speed_reward = C.COST_WEIGHT_SPEED_REWARD * action[:, 0]
        
        # Backward cost: Humanoids are more stable moving forward; penalize reverse motion
        backward_cost = C.COST_WEIGHT_BACKWARD * torch.relu(-action[:, 0]) ** 2
        
        # Rotation penalty: Smooth out sharp jittery turns
        omega_cost = C.COST_WEIGHT_ROTATION * action[:, 2] ** 2

        # --- 4. PATH TRACKING (MODIFIED) ---
        ...
        if self.current_path_segment is not None and self.current_path_segment.shape[0] > 0:
            # Compute distance only to the nearest 20 points 
    ...
    def command(self, state_tensor):
        ...
        Output: Optimal control [vx, vy, omega] for the next timestep.
        """
        if self.global_path is not None:
            ...
            if state_tensor.dim() == 1:
                ...
                curr_pos = state_tensor[:, :2] # Take all batches, only first 2 columns (x,y)
            ...
            #  Compute distances from current position to ALL global waypoints
